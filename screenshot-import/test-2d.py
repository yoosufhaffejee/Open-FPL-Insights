...
def find_name_plates(image_path):
    ...
    # 1. Find Y-clusters (rows of name plates)
    white_rows = [0] * height
    ...
    # 2. For each Y-cluster, find X-clusters
    for y_c in y_clusters:
        ...
        x_clusters = []
        curr_x = None
        # A valid column should have white pixels in at least 30% of the Y-cluster height
        ...
        # Filter X-clusters
        min_width = width * 0.05 # A name plate is at least 5% of image width
...
for file in os.listdir(folder):
    ...
    try:
        plates = find_name_plates(path)
        print(f"\n{file}: Found {len(plates)} plates")
